...
limma_fit.LNCaP <- filter(limma_fit.LNCaP, fdr <= 0.05, abs(estimate) > log2(1.3))
...
clustering_combat <- expr.rma.combat %>% 
                     ...
                     dist() %>% 
                     hclust(method = "complete")
plot(clustering_combat)

limma_fit.LNCaP.combat <- fit_limma(expr.rma.combat[ , 1:6], design.mat.LNCaP)  
